...
try {
    ...
    $stmt_latest = $conn->query("SELECT temp, humidity, pressure, light_level, light_category, created_at FROM datasensor ORDER BY id DESC LIMIT 1");
    ...
    $stmt_history = $conn->query("
        SELECT temp, humidity, pressure, light_level, light_category, created_at
        FROM datasensor
